...
public class Prefs {
    ...
    static String getCoin(Context context, int widgetId) {
        String value = getValue(context, widgetId, COIN);
        if(value == null) return context.getString(R.string.default_coin);
        return value;
    }
	...
	static void setValues(Context context, int widgetId, String coin, String currency, int refreshValue, int provider, boolean checked, String theme, boolean iconChecked) {
    ...
    private static JSONObject convert(Context context, int widgetId, String string) {
        ...
        JSONObject obj = new JSONObject();
        try {
            if(strings.length > 0) obj.put(COIN, strings[0]);
            if(strings.length > 1) obj.put(CURRENCY, strings[1]);
            if(strings.length > 2) obj.put(REFRESH, strings[2]);
            if(strings.length > 3) obj.put(PROVIDER, strings[3]);
            if(strings.length > 4) obj.put(SHOW_LABEL, strings[4]);
            if(strings.length > 5) obj.put(HIDE_ICON, strings[5]);
        } catch (JSONException e) {
            e.printStackTrace();
        }
        getPrefs(context).edit().putString("" + widgetId, obj.toString()).commit();
