	...
	public void onReceive(final Context context, final Intent intent) {
		new Thread(new Runnable() {
			...
			public void run() {
				...
				int coin = Integer.valueOf(Prefs.getCoin(context, appWidgetId));
				...
				AltcoinInterface provider = CoinFactory.getEnum(coin)[providerInt];

				try {
					String amount = provider.getValue(currencyCode);
					WidgetViews.setText(context, views, coin, currency, amount, true, provider.getLabel(), appWidgetId);
					...
					WidgetViews.setText(context, views, coin, currency, null, !isOld, provider.getLabel(), appWidgetId);
				}
